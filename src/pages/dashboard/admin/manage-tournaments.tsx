...
export function AdminTournamentsPage() {
  ...
  useEffect(() => {
    ...
    // permitir disparar reload de dentro do modal
    (window as any).refreshTournaments = loadTournaments;

    return () => {
      delete (window as any).refreshTournaments;
    };
  }, []);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-6">
        <div>
          ...
          <p className="text-zinc-400">
            Create and organize competitive events.
          </p>
        </div>
        <Button
      ...
      <div className="grid gap-4">
        {tournaments.map((t) => (
          <Card
            key={t.id}
            className="bg-zinc-900/40 border-white/10 hover:border-white/20 transition-all"
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-zinc-500 mt-1">
                    ...
                    <span className="flex items-center gap-1">
                      <Users className="h-3 w-3" /> {t.teams}/{t.maxTeams} Teams
                    </span>
                  </div>
                </div>
              </div>
