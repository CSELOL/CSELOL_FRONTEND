      ...
      <div className="container max-w-6xl px-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
            <div>
                ...
                <p className="text-zinc-400">Compete in official leagues and community cups.</p>
            </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tournaments.map((t) => (
                <Card key={t.id} className="bg-zinc-900/50 border-white/10 overflow-hidden flex flex-col hover:border-white/20 transition-all group">
                    <div className="h-48 w-full relative overflow-hidden">
                         <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-110" style={{ backgroundImage: `url(${t.img})` }} />
                         <div className="absolute inset-0 bg-gradient-to-t from-zinc-900 via-transparent to-transparent" />
                         <Badge className={`absolute top-4 right-4 ${
                             t.status === "OPEN" ? "bg-emerald-500 text-white" : 
                             t.status === "FULL" ? "bg-yellow-500 text-black" : "bg-zinc-700 text-zinc-400"
                         }`}>
                             {t.status}
                         </Badge>
                    </div>
                    ...
                    <CardContent className="space-y-3 flex-1">
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="font-bold text-white font-mono">{t.prize}</span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                    ...
                    <CardFooter className="pt-0">
                        ...
                        */}
                        {t.status === "OPEN" ? (
                            ...
                            <Button variant="outline" className="w-full border-white/10 text-zinc-500" disabled>
                                <Lock className="mr-2 h-4 w-4" /> 
                                {t.status === "FULL" ? "Registration Closed" : "Event Ended"}
                            </Button>
                        )}
                    </CardFooter>
                </Card>
