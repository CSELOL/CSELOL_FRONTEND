...
const sponsors = [
  ...
  { name: "Monster Energy", domain: "monsterenergy.com", tier: "Beverage Partner" },
...
export function SponsorsSection() {
  return (
    <section className="w-full bg-background border-y border-white/5 py-24 overflow-hidden relative">
      ...
      <div className="container px-4 md:px-6 mb-12 text-center relative z-10">
        <h2 className="text-3xl font-bold tracking-tight text-white sm:text-4xl uppercase">
            Official Partners
        </h2>
        <p className="mt-4 text-zinc-400 max-w-2xl mx-auto">
            The CSELOL Circuit is powered by industry leaders supporting the growth of esports in Sergipe.
        </p>
      </div>
         ...
         <div className="flex animate-infinite-scroll gap-6 px-6 w-max">
            {[...sponsors, ...sponsors].map((sponsor, index) => (
                <Link to="/sponsors" key={index}>
                    <Card 
                        className="w-[280px] h-[160px] bg-zinc-900/40 border-white/5 flex flex-col items-center justify-center gap-4 hover:bg-white/5 hover:border-primary/30 transition-all duration-300 group cursor-pointer backdrop-blur-sm"
                    >
